...
#ionic_liquid_name="C10MIM+_TFSI-_model1"
ionic_liquid_name="PositiveNeutralDoubleDimerMinusDimer"
chi_parameter=0.71
Epsilon_r=14.0
Epsilon_LJ_particle_particle=20 # ( = epsilon_LJ * k_{B})
...
positive_bead_charge=0.5
negative_bead_charge=-0.5
Donan_potential_intial_guess=0.8
string_length=1.2			#
...
./bin/runC4MIMBF4TEST.x <<EOF
$test_file_stub
EOF

write_dir="./run_results/compare_epsilonLJ/charge/${Epsilon_LJ_particle_particle}-${Epsilon_LJ_particle_wall}_${ionic_liquid_name}TEST2"
mkdir -p ${write_dir}
