...
module functionalderivatives
  ...
  public :: van_der_waals_density_indept_integrand_centre_to_centre_r8
  public :: van_der_waals_density_indept_integrand_centre_to_centre_r4

  real(dp) :: CURRENT_BULK_BEAD_DENSITY

contains

  function calculate_centre_to_centre_functional_deriv(calculate_bulk, size_of_terms, cation_or_anion, type_of_interaction, n_cation_centre, n_anion_centre)
    logical, intent(in) :: calculate_bulk
    integer, intent(in) :: size_of_terms
    character(len=*), intent(in) :: cation_or_anion
    character(len=*), intent(in) :: type_of_interaction
    ...
    if(calculate_bulk) then
       ...
       if(trim(cation_or_anion) == 'c' .or. trim(cation_or_anion) == 'a') then
          ...
          if(trim(type_of_interaction) == 'r4') then
             calculate_centre_to_centre_functional_deriv(:) = (-1.0_dp * epsilon_eighth_power_const * (1.0 / beta) * (hs_diameter**4) * 2.0_dp * pi * ns_bulk(:)) * ( &
                  (2.0_dp/(hs_diameter)))
          else if(trim(type_of_interaction) == 'r8') then
             ...
             print *, "invalid value for type_of_interaction = ", trim(type_of_interaction)
             print *, "aborting..."
             call abort()
          end if
       ...
       if(trim(type_of_interaction) == 'r4') then
          ...
          if(trim(cation_or_anion) == 'c') then
             calculate_centre_to_centre_functional_deriv = -1.0_dp * epsilon_eighth_power_const * (1.0_dp / beta) * (hs_diameter**4.0_dp) * (1.0_dp) * &
                  pi * integrate_z_cylindrical(n_anion_centre, van_der_waals_density_indept_integrand_centre_to_centre_r4, "all_z")
          else if(trim(cation_or_anion) == 'a') then
             calculate_centre_to_centre_functional_deriv = -1.0_dp * epsilon_eighth_power_const * (1.0_dp / beta) * (hs_diameter**4.0_dp) * (1.0_dp) * &
                  pi * integrate_z_cylindrical(n_cation_centre, van_der_waals_density_indept_integrand_centre_to_centre_r4, "all_z")
          else
             print *, "functionalderivatives.f90: calculate_centre_to_centre_functional_deriv"
             print *, "Invalid value of trim(cation_or_anion) = ", trim(cation_or_anion)
             print *, "Must be 'c' or 'a'"
          ...
          if(trim(cation_or_anion) == 'c') then
             calculate_centre_to_centre_functional_deriv = -1.0_dp * epsilon_eighth_power_const * (1.0_dp / beta) * (hs_diameter**8.0_dp) * (1.0_dp) * &
                  2.0_dp * pi * integrate_z_cylindrical(n_anion_centre, van_der_waals_density_indept_integrand_centre_to_centre_r8, "all_z")
          else if(trim(cation_or_anion) == 'a') then
             calculate_centre_to_centre_functional_deriv = -1.0_dp * epsilon_eighth_power_const * (1.0_dp / beta) * (hs_diameter**8.0_dp) * (1.0_dp) * &
                  2.0_dp * pi * integrate_z_cylindrical(n_cation_centre, van_der_waals_density_indept_integrand_centre_to_centre_r8, "all_z")
          else
             print *, "functionalderivatives.f90: calculate_centre_to_centre_functional_deriv"
             print *, "Invalid value of trim(cation_or_anion) = ", trim(cation_or_anion)
             print *, "Must be 'c' or 'a'"
...
function van_der_waals_density_indept_integrand_centre_to_centre_r8(z, xi_in)
 integer, intent(in) :: z
 integer, intent(in) :: xi_in
 real(dp)            :: van_der_waals_density_indept_integrand_centre_to_centre_r8
 ...
 if(abs(xi_int) >= n_discretised_points_z) then
    van_der_waals_density_indept_integrand_centre_to_centre_r8 = 1.0_dp / (6.0_dp * (real(xi_real,dp)**6.0_dp))
 else
    van_der_waals_density_indept_integrand_centre_to_centre_r8 = 1.0_dp / (6.0_dp * (hs_diameter**6.0_dp))
 end if

 return
end function van_der_waals_density_indept_integrand_centre_to_centre_r8

function van_der_waals_density_indept_integrand_centre_to_centre_r4(z, xi_in)
 integer, intent(in) :: z
 integer, intent(in) :: xi_in
 real(dp)            :: van_der_waals_density_indept_integrand_centre_to_centre_r4
 ...
 if(abs(xi_int) >= n_discretised_points_z) then
    van_der_waals_density_indept_integrand_centre_to_centre_r4 = 1.0_dp / (real(xi_real,dp)**2.0_dp)
 else
    van_der_waals_density_indept_integrand_centre_to_centre_r4 = 1.0_dp / (hs_diameter**2.0_dp)
 end if

 return
end function van_der_waals_density_indept_integrand_centre_to_centre_r4
